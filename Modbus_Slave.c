...
#endif
{
    ...
    switch (MB->Seq) {
        ...
        case 10:
            ...
            if (Ticks > MB->Ticks)
#else
            if (MB->InnerTicks > MB->RX_Timeout)
#endif
            {
                MB->RX_LEN = 0;
                MB->Seq = 5;
                ...
                #else
                                        MB->InnerTicks = 0; // reset the inner tick counter
                #endif
                                        MB->Seq = 30;
                                    }
                                } else {*/
        ...
        case 20:
            if (MB->RX_LEN >= MB->RX_MSG_LEN) {
                ...
                //prepare the bus inter-frame delay
#ifndef _MB_TICKLESS
                MB->Ticks = Ticks + MB->RX_Silent_Interval_MS;
#else
                MB->InnerTicks = 0; // reset the inner tick counter
                ...
                switch (MB->MB_Stat) {
                    case MB_STAT_OK:
                        //used when the MB is in high speed mode,(Ethernet/USB/etc...)
                        //zero wait state in case of 1 device is attached to the network
                        if (MB->RX_Silent_Interval_MS == 0) {
                            MB->hw_interface.MB_Activate_TX(1); // activate transmitter
                            MB->hw_interface.MB_Transmit(MB->TxBuffer, MB->TX_MSG_LEN);
                            // disable TX mode in case of autocomplete (non blocking call)
                            ...
                            //move to processing the delay before the transmission
                            MB->Seq = 30;
                        }
                        break;

                        //in case the message was directed to another slave, then need to wait till the other slave replies back
                    case MB_RX_ERR_ADD:
                        MB->Seq = 35;
                        break;
...
#else
            else if (MB->InnerTicks > MB->RX_Timeout)
#endif
            {
        ...
        case 30:
            //process the inter frame time
#ifndef _MB_TICKLESS
            if (Ticks > MB->Ticks)
#else
            if (MB->InnerTicks > MB->RX_Silent_Interval_MS)
#endif
            {
                MB->hw_interface.MB_Activate_TX(1); // activate transmitter
                MB->hw_interface.MB_Transmit(MB->TxBuffer, MB->TX_MSG_LEN);
            ...
            //wait for the silent interval to re-enable reception
        case 35:
            MB->Seq = 0;
            break;
